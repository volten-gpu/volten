...
:::note[Thread count]
Volten can often infer how many invocations to run from one clear input buffer. If a pass has multiple possible input buffers, only raw buffers, or a shape that is not one thread per element, set `threads` on the kernel or pass.
:::
