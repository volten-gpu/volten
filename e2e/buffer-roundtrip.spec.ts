...
test.describe('Buffer data round-trip', () => {
    ...
    test('f32 data survives GPU round-trip', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const node = identityF32({ inout: buf });
            v.run(node);
    ...
    test('i32 data with negative values survives GPU round-trip', async ({
        page
    }) => {
        const result = await page.evaluate(async () => {
            const { volten, Buffer, kernel } = (window as any).Volten;
    ...
    test('larger buffer (1024 elements) round-trips correctly', async ({
        page
    }) => {
        const result = await page.evaluate(async () => {
            const { volten, Buffer, kernel } = (window as any).Volten;
            ...
            const buf = new Buffer(data, 'f32', 'rw');
            const identityLargeBuffer = kernel({
                shader: `
              fn main(gid: vec3u) {
                inout[gid.x] = inout[gid.x];
              }
            ...
            const node = identityLargeBuffer({ inout: buf });
            v.run(node);
            const output = await v.read(buf);
    ...
    test('reading a Buffer directly (not through a Node) works', async ({
        page
    }) => {
        const result = await page.evaluate(async () => {
            const { volten, Buffer, kernel } = (window as any).Volten;
