...
test.describe('Linear chain execution', () => {
    ...
    test('A → B → C: three-node chain', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            `, { outputs: { out: { definedBy: 'src' } }, threads: 'src' });
            ...
            const A = v.pass(KA, { src: buf1, out: buf2 });
            const B = v.pass(KB, { src: A.out, out: buf3 });
            const C = v.pass(KC, { src: B.out, out: buf4 });
...
test.describe('Multi-node independent execution', () => {
    ...
    test('v.run([A, B]) executes two independent nodes', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const A = v.pass(K1, { inout: buf1 });
            const B = v.pass(K2, { inout: buf2 });
...
test.describe('Shared buffer execution ordering', () => {
    ...
    test('v.run([A, B]) with shared rw buffer: A runs before B', async ({ page }) => {
        const result = await page.evaluate(async () => {
            const { volten, Buffer, Kernel } = (window as any).Volten;
            const v = await volten();
            ...
            const A = v.pass(KA, { inout: shared });
            const B = v.pass(KB, { inout: shared });
