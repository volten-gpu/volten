...
test.describe('Resource features', () => {
    ...
    test('Uniform bindings affect compute output and can be updated', async ({
        ...
        const result = await page.evaluate(async () => {
            ...
            const node = scaleValues({ input, output, scale });
            await v.wait(node);
    ...
    test('readDebug decodes logs written by a real shader', async ({
        page
    }) => {
        const result = await page.evaluate(async () => {
            const { volten, Buffer, Uniform, kernel } = (window as any).Volten;
            ...
            return {
                ...
                logs: debug.logs.map((log: any) => ({
                    ...
                    gid: [...log.gid],
                    message: log.message,
                    value: Array.isArray(log.value) ? [...log.value] : log.value
                }))
            };
        });
